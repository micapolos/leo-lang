...
fun <T> Array<T>.at(index: Int): T =
	branchPtr.arrayAt(index, arrayStartMask) as T? ?: default

fun <T> Array<T>.put(index: Int, value: T): Array<T> =
	copy(branchPtr = branchPtr.arrayPut(index, arrayStartMask, if (value == default) null else value))
...
fun <T, R> R.foldIndexed(array: Array<T>, fn: R.(IndexedValue<T>) -> R): R =
	foldArrayIndexed(array.branchPtr, 0, arrayStartMask) {
