...
data class NativeBody<V : Any, A : Any>(val native: TypeNative<V, A>) : Body<V>()
...
fun <V : Any, A : Any> body(native: TypeNative<V, A>): Body<V> = NativeBody(native)
...
fun <V : Any> Body<V>.script(value: V): Script =
	when (this) {
		...
		is NativeBody<*, *> -> (native as TypeNative<V, Any>).bodyScript(value as Any)
	}
...
fun <V : Any> Body<V>.unsafeValue(script: Script): V =
	when (this) {
		...
		is NativeBody<*, *> -> (native as TypeNative<V, Any>).unsafeBodyValue(script)
	}
