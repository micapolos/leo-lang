...
fun <V> Script<V>.select(selector: Selector): Script<V>? =
	orNull.fold(selector.getterStreamOrNull) { selectorItem ->
		this?.get(selectorItem)
	}

fun <V> Script<V>.select(selectorScript: Script<Selector>): Script<V>? =
	when (selectorScript) {
		is MetaScript -> select(selectorScript.meta.value)
		is InvokeScript -> selectorScript.lhsOrNull?.let { lhsSelectorScript ->
			select(lhsSelectorScript)
		}.invoke(selectorScript.word, selectorScript.rhsOrNull?.let { rhsSelectorScript ->
			select(rhsSelectorScript)
		})
	}
