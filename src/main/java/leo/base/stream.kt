...
fun <V> V.onlyStreamThen(nextOrNullFn: () -> Stream<V>?): Stream<V> =
	Stream(this) { nextOrNullFn() }

fun <V> Stream<V>.then(nextOrNullFn: () -> Stream<V>?): Stream<V> =
	first.onlyStreamThen { nextOrNull?.then(nextOrNullFn) ?: nextOrNullFn() }
...
fun <V, R> Stream<V>.map(fn: (V) -> R): Stream<R> =
	fn(first).onlyStreamThen { nextOrNull?.map(fn) }
...
fun <V, R : Any> Stream<V>.filterMap(fn: (V) -> R?): Stream<R>? {
	...
	return mappedFirst?.onlyStreamThen { nextOrNull?.filterMap(fn) } ?: nextOrNull?.filterMap(fn)
}
...
fun <V> Stream<V>.intersperse(separator: V): Stream<V> =
	first.onlyStreamThen {
		nextOrNull?.map { separator.onlyStreamThen { it.onlyStream } }?.join
	}
